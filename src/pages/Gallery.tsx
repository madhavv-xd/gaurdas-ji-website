          ...
          <ul className="columns-2 md:columns-3 gap-3 sm:gap-4 [&>li]:mb-3 sm:[&>li]:mb-4">
            {PHOTOS.map((src, i) => (
              <li key={src} className="break-inside-avoid">
                <button onClick={() => setOpen(i)} className="group block w-full rounded-xl overflow-hidden bg-cream-200 min-h-[120px]" aria-label={`Open photo ${i + 1} of ${PHOTOS.length}`}>
                  <img src={src} alt="" loading="lazy" decoding="async" className="w-full h-auto group-hover:scale-105 transition-transform duration-700" />
                </button>
              </li>
            ))}
          </ul>
